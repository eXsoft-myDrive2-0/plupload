...
function document_fileUpload() {
	...
	if (uploader.files.length > 0) {
		...
		// 컨텐트 Submit.
		isSuccess = document_submitContent();
	}
...
function document_submitContent() {
	...
	$("#document_attrs .content_detail_content").each(function(index, item) {
		...
		if ((first_tag.is("input[type=text]") && !first_tag.is(".search_calendar")) || first_tag.is("textarea") || first_tag.is("select") || first_tag.is("input[type=password]")) {
		...
		// DATE의 경우.
		else if (first_tag.is("input[type=text]") && first_tag.is(".search_calendar")) {
			attr.docTypeAttrId = first_tag.attr("id").replace("document_attr_", "").replace("_from", "");
			
			$("input[type=text]", item).each(function(index, item) {
				
				if($(item).val() == ""){
					createMessageModal({
						title: null,
						type: "warning",
						message: $.i18n.prop("SUBMIT_DOCUMENT_FAIL_DATE_IS_REQUIRED"),			
						ok_callback: function(){$(item).focus();},
						size: null
					});
					existValue = false;
					return;
