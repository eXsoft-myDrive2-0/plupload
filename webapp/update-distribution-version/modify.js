 ...
 * @param key
 */
function distribution_selectContentDetailForUpdating(key) {
	// 모달 세팅.
	contentModal.dialog({     
		title: $.i18n.prop("LABEL_MODIFY_DISTRIBUTION_FILE2"),
		buttons: [
			{ 
				...
				click: function() {
					...
					distribution_plupload_submit_version(key);
				}
			}, 
			{
	...
	$("#document_uploader").pluploadQueue({
		...
		init: {
			...
			UploadComplete: function(up, files) {
				// 컨텐트 Submit.
				distribution_submitContent(key);
       		}
		}
	});
...
let is_submit = false;
function distribution_plupload_submit_version(key) {
	if(is_submit == true){
		return;
	}
	...
	if(validate(uploader)){
		...
		if (uploader.files.length == 1){
			...
			// 컨텐트 Submit.
			isSuccess = distribution_submitContent(key);
		}
 ...
 * 컨텐트 Submit.
 */
function distribution_submitContent(key) {
	// 파일 목록 생성.
	let uploader = $("#document_uploader").pluploadQueue();
	let files = [];
	var distributionId = key.distributionId;
	...
	apiExecute(
		"POST",
		"/distributions/versions/"+distributionId,
		{		
			version: $("#distribution_version").text().trim(),
			updateHistory: $("#document_description").val(),
			files: files
		},
		function(){
			createLoadingModal({
